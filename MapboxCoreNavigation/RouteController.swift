...
open class RouteController: NSObject, Router {
    ...
    public init(along route: Route, directions: Directions = Directions.shared, locationManager: NavigationLocationManagerProtocol, eventsManager eventsOverride: EventsManager? = nil) {
        ...
        self.locationManager.navDelegate = self
        resumeNotifications()
    ...
    @objc public func suspendLocationUpdates() {
      print("===:==== calling stopUpdatingLocation")
...
extension RouteController: MGLLocationManagerDelegate {
  public func locationManagerShouldDisplayHeadingCalibration(_ manager: MGLLocationManager) -> Bool {
    return false
  }
    ...
    @objc func interpolateLocation() {
        ...
        self.locationManager(locationManager, didUpdate: [interpolatedLocation])
    }
  
  @objc public func locationManager(_ manager: MGLLocationManager, didFailWithError error: Error) {
   
  }
  
  @objc public func locationManager(_ manager: MGLLocationManager, didUpdate newHeading: CLHeading) {
        heading = newHeading
    }

  @objc public func locationManager(_ manager: MGLLocationManager, didUpdate locations: [CLLocation]) {
